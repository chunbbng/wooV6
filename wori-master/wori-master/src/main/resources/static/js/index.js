...
// 작업 유형에 따른 슬라이더 업데이트 및 노란색 평균 범위 표시
...
secondSlider.noUiSlider.on('update', function (values, handle) {
    document.getElementById('secondTimeValue').textContent = values[handle];
...
expandButton.addEventListener('click', function() {
    if (secondSliderContainer.style.display === 'none') {
        secondSliderContainer.style.display = 'block';
        expandButton.textContent = '시간 축소';
    } else {
        secondSliderContainer.style.display = 'none';
        expandButton.textContent = '시간 확장';
    }
});